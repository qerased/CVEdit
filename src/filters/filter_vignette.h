...
public:
    ...
    {
        if (mat.empty () || strength_ <= 0.) return;

        int w = mat.cols;
        int h = mat.rows;

        if (mask_.empty () || mask_.rows != h || mask_.cols != w || cached_radius_ != radius_)
        {
            build_mask (w, h, radius_);
        }
        ...
        cv::Mat scale;
        scale = 1.0f - strength_ * (1.0f - mask_);
    ...
    void set_strength (float strength) { strength_ = std::clamp (strength, 0.f, 1.f); }
    void set_radius (float radius) { radius_ = std::clamp (radius, 0.f, 0.999f); }
private:
